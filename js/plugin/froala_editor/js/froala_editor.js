...
function(a) {
	...
	a.fn.editable = function(c) {
		...
		return "object" != typeof c && c ? void 0 : this.each(function() {
			/*****************2016.4.12 修正七牛上传的图片初次加载路径报错，初次加载去除原有的src路径********************/
			var z = $(this).html().replace(/http:\/\/7xnhbn.*?'/g,"'");
			$(this).html(z);
			/*************** end **********************/
			var d = this,
				e = a(d),
	...
	a.Editable.image_commands = {
		...
		d.onerror = a.proxy(function() {
			...
			a.attr("src",b), 
			this.hide(), this.hideImageLoader(), this.$image_editor.show(), this.saveUndoStep(), this.enable(), this.triggerEvent("imageReplaced", [a]), setTimeout(function() {
				a.trigger("click")
			}, 0)
		...
		var d = '<img class="fr-fin" data-fr-image-preview="' + c + '" alt="Image" title '+(this.y?"role=qnImg isrc="+this.y.name:"")+' src="' + b + '" width="' + this.options.defaultImageWidth + '">',
		...
		try {
			var c = a.parseJSON(b);
			this.options.qncb && (c.link = this.options.qncb(c));
